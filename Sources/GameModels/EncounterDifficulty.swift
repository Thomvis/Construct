...
public struct EncounterDifficulty {
    public let party: [PartyEntry] // PC levels
    public let monsters: [Fraction] // Challenge ratings

    public let partyThresholds: EncounterDifficultyThresholds
    public let adjustedXp: Int // A value used to measure an encounter difficulty (not to reward the players)

    public init?(party: [Int], monsters: [Fraction]) {
        self.init(party: party.map { .init(level: $0, name: nil) }, monsters: monsters)
    }

    public init?(party: [PartyEntry], monsters: [Fraction]) {
    ...
    public var percentageOfDeadly: CGFloat {
        CGFloat(adjustedXp) / CGFloat(partyThresholds.deadly)
    }

    public func percentageOfDeadly(_ category: Category) -> CGFloat {
        CGFloat(partyThresholds.value(for: category)) / CGFloat(partyThresholds.deadly)
    }

    public enum Category: String, CaseIterable, Equatable, Identifiable {
        case easy
        case medium
        case hard
        case deadly
...
public struct EncounterDifficultyThresholds {
    public let easy: Int
    public let medium: Int
    public let hard: Int
    public let deadly: Int

    public func value(for category: EncounterDifficulty.Category) -> Int {
        switch category {
    ...
    public static func +(lhs: EncounterDifficultyThresholds, rhs: EncounterDifficultyThresholds) -> EncounterDifficultyThresholds {
    ...
    public static var zero = EncounterDifficultyThresholds(easy: 0, medium: 0, hard: 0, deadly: 0)
}

public let crToXpMapping: [Fraction: Int] = [
