...
public func SimpleButton<Label>(action: @escaping () -> Void, @ViewBuilder label: () -> Label) -> some View where Label: View {
    Button(action: action, label: label).buttonStyle(SimpleButtonStyle())
}

public func SimpleAccentedButton<Label>(action: @escaping () -> Void, @ViewBuilder label: () -> Label) -> some View where Label: View {
