    ...
    func onTap(_ stats: StatBlock) -> ((StatBlockView.TapTarget) -> Void)? {
        { target in
            ...
            case .ability(let a):
                ...
                self.viewStore.send(.popover(.rollCheck(.rolling(.abilityCheck(modifier, ability: a, skill: nil, creatureName: stats.name, environment: self.env.diceRollerEnvironment), rollOnAppear: true))))
            case .skill(let s):
                let modifier: Int = stats.skillModifier(s)?.modifier ?? 0
                self.viewStore.send(.popover(.rollCheck(.rolling(.abilityCheck(modifier, ability: s.ability, skill: s, creatureName: stats.name, environment: self.env.diceRollerEnvironment), rollOnAppear: true))))
