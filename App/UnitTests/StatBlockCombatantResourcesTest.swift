...
class StatBlockCombatantResourcesTest: XCTestCase {
    ...
    func testSpellcasting() {
        ...
        monster.stats.features = [CreatureFeature(id: UUID(), name: "Spellcasting", description: "The naga is an 11th-level spellcaster. Its spellcasting ability is Wisdom (spell save DC 16, +8 to hit with spell attacks), and it needs only verbal components to cast its spells. It has the following cleric spells prepared:\n\n• Cantrips (at will): mending, sacred flame, thaumaturgy\n• 1st level (4 slots): command, cure wounds, shield of faith\n• 2nd level (3 slots): calm emotions, hold person\n• 3rd level (3 slots): bestow curse, clairvoyance\n• 4th level (3 slots): banishment, freedom of movement\n• 5th level (2 slots): flame strike, geas\n• 6th level (1 slot): true seeing")]
    ...
    func testInnateSpellcasting() {
        var monster = Fixtures.monster
        monster.stats.features = [CreatureFeature(id: UUID(), name: "Innate Spellcasting", description: "The giant's innate spellcasting ability is Charisma. It can innately cast the following spells, requiring no material components:\n\nAt will: detect magic, fog cloud, light\n3/day each: feather fall, fly, misty step, telekinesis\n1/day each: control weather, gaseous form")]
            .map { apply(ParseableCreatureFeature(input: $0)) { $0.parseIfNeeded() } }
            .identified
        monster.stats.actions = []
    ...
    func testRechargingFeature() {
        var monster = Fixtures.monster
        monster.stats.features = [CreatureFeature(id: UUID(), name: "Relentless (Recharges after a Short or Long Rest)", description: "If the boar takes 7 damage or less that would reduce it to 0 hit points, it is reduced to 1 hit point instead.")]
            .map { apply(ParseableCreatureFeature(input: $0)) { $0.parseIfNeeded() } }
            .identified
        monster.stats.actions = []
